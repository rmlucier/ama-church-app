  ...
  return (
    <main className="bg-[#F5F5F2] min-h-screen p-8">
      <div className="max-w-6xl mx-auto">
        <h1 className="text-4xl font-bold text-center mb-6 text-[#7A6A53]">Get Help in Albion</h1>
        ...
        {/* Loading / Error / Empty */}
        {loading && <p className="text-center text-[#7A6A53]">Loading resources...</p>}
        {error && <p className="text-center text-red-600">{error}</p>}
        {!loading && !error && resources.length === 0 && (
          <p className="text-center text-[#7A6A53]">No resources found.</p>
        )}
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {resources.map((resource, idx) => (
            <div key={idx} className="bg-white rounded-xl shadow-md p-4 space-y-2 print:break-inside-avoid">
              <h2 className="text-xl font-semibold text-[#7A6A53]">{resource.Name}</h2>
              <p className="text-[#948C75] font-medium">{resource.Type}</p>
              <p className="text-[#7A6A53]">{resource.Description}</p>
              {resource['Contact Name'] && (
                <p className="text-[#7A6A53]">Contact: {resource['Contact Name']}</p>
              )}
              ...
              {resource.Email && (
                <p className="text-[#7A6A53]">
                  ...
                  <a
                    ...
                    className="underline text-[#99B2B7] hover:text-[#7A6A53]"
                  >
                    {resource.Email}
                  </a>
                </p>
              ...
              {resource.Address && (
                <>
                  <p className="text-[#7A6A53]">Address: {resource.Address}</p>
                  <a
                    href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(resource.Address)}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    className="text-[#99B2B7] underline text-sm hover:text-[#7A6A53]"
                  >
                    Get Directions
                  </a>
                </>
